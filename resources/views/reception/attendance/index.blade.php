...
<div class="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6">
            <div class="flex items-center">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{{ $todayCount ?? 0 }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6">
            <div class="flex items-center">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{{ $uniqueCourses ?? 0 }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-base md:text-lg lg:text-xl font-bold text-gray-900 truncate">
                        {{ $latestAttendance ? $latestAttendance->attended_at->format('h:i A') : 'No scans yet' }}
                    </p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6 mb-4 md:mb-6">
        ...
        <form method="GET" action="{{ route('reception.attendance.index') }}">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4 bg-gray-50 border-b border-gray-200 flex flex-col sm:flex-row sm:justify-between sm:items-center">
            <div class="mb-2 sm:mb-0">
                ...
                <p class="text-xs md:text-sm text-gray-600 mt-0.5">Today: {{ now()->format('F d, Y') }}</p>
            </div>
            @if(isset($attendances) && count($attendances))
            <div class="text-xs md:text-sm text-gray-700">
                <span class="font-semibold">{{ $attendances->total() }}</span> records
            </div>
            @endif
        </div>
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-2 mt-0.5 md:mt-1">
                                <span class="text-xs md:text-sm text-gray-600">
                                    {{ count($records) }} attendance{{ count($records) > 1 ? 's' : '' }}
                                </span>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-2 md:ml-3 lg:ml-4 min-w-0">
                                            <div class="text-xs md:text-sm font-medium text-gray-900 truncate">{{ $attendance->student->name }}</div>
                                            <div class="text-xs text-gray-500 truncate">{{ $attendance->student->registration_number }}</div>
                                            <!-- Mobile: Show course and contact -->
                                            <div class="md:hidden mt-1">
                                                <div class="text-xs text-gray-700 truncate">{{ $attendance->student->course->name ?? 'N/A' }}</div>
                                                <div class="text-xs text-gray-500 truncate">{{ $attendance->student->student_phone }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap hidden md:table-cell">
                                    <div class="text-xs md:text-sm text-gray-900 truncate">{{ $attendance->student->student_phone }}</div>
                                </td>
                                
                                <!-- Course -->
                                <td class="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4 hidden lg:table-cell">
                                    <div class="text-xs md:text-sm font-medium text-gray-900 truncate">{{ $attendance->student->course->name ?? 'N/A' }}</div>
                                </td>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <div class="text-xs text-gray-500 truncate">
                                                {{ $attendance->attended_at->format('M d') }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                
                                @foreach($attendance->payment_status_by_month as $status)
                                <td class="px-2 md:px-3 lg:px-4 py-2 md:py-3 lg:py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-1.5 py-0.5 md:px-2 md:py-1 rounded-full text-xs font-medium {{ $status == 'Paid' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        @if($status == 'Paid')
            ...
            <div class="px-3 md:px-4 lg:px-6 py-2 md:py-3 lg:py-4 bg-gray-50 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs md:text-sm text-gray-700 mb-3 sm:mb-0">
                        @if($attendances->total())
                        Showing <span class="font-semibold">{{ $attendances->firstItem() }}</span> to 
                        <span class="font-semibold">{{ $attendances->lastItem() }}</span> of 
                        <span class="font-semibold">{{ $attendances->total() }}</span> results
                        @endif
                    </div>
                    
                    <div class="flex items-center">
                        {{ $attendances->appends(request()->query())->onEachSide(1)->links() }}
                    </div>
                </div>
            </div>
        @else
