                        ...
                        @for($m = 1; $m <= 4; $m++)
                            <th colspan="4" class="px-2 md:px-4 py-2 md:py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider border-l">
                                <div class="flex flex-col items-center">
                                    <span class="font-bold text-gray-700 text-xs md:text-sm">Month {{ $m }}</span>
                                    <span class="text-xs md:text-sm text-gray-500 font-normal mt-0.5 md:mt-1">W{{ (($m-1)*4)+1 }}-{{ $m*4 }}</span>
                                </div>
                            </th>
                        @endfor
                    ...
                    <tr class="bg-gray-100">
                        @for($m = 1; $m <= 4; $m++)
                            <th class="px-1.5 md:px-3 py-1.5 md:py-2 text-center text-xs font-medium text-gray-600 border-l">
                                <div class="text-xs md:text-sm">M{{ $m }}</div>
                            </th>
                        @endfor
                        ...
                        @for($w = 1; $w <= 16; $w++)
                            <th class="px-1.5 md:px-3 py-1.5 md:py-2 text-center text-xs font-medium text-gray-600 border-l">
                                <div class="flex flex-col items-center">
                                    <span class="font-semibold text-xs md:text-sm">W{{ $w }}</span>
                                </div>
                            </th>
                        @endfor
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap sticky left-[80px] md:left-[150px] bg-white z-10 border-r min-w-[70px] md:min-w-[120px]">
                            <div class="text-xs md:text-sm text-gray-900 truncate">{{ $student->course->name ?? '-' }}</div>
                        </td>
                        ...
                        <td class="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap sticky left-[150px] md:left-[270px] bg-white z-10 border-r min-w-[70px] md:min-w-[120px]">
                            <div class="text-xs md:text-sm text-gray-900 truncate">{{ $student->center->name ?? '-' }}</div>
                        </td>
                        ...
                        @for($m = 1; $m <= 4; $m++)
                            <td class="px-1.5 md:px-3 py-2 md:py-3 text-center border-l">
                                <span class="px-1.5 md:px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ ($paymentStatus[$m] ?? 'Unpaid') == 'Paid' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <td class="px-2 md:px-4 py-2 md:py-3 whitespace-nowrap text-center border-l">
                            <div class="flex items-center justify-center space-x-1.5 md:space-x-2">
                                <div class="w-12 md:w-20 bg-gray-200 rounded-full h-1.5 md:h-2">
                                    <div class="bg-green-600 h-1.5 md:h-2 rounded-full" style="width: {{ min($percent, 100) }}%"></div>
                                </div>
                                <span class="text-xs md:text-sm font-semibold {{ $percent >= 75 ? 'text-green-600' : ($percent >= 50 ? 'text-yellow-600' : 'text-red-600') }}">
                                    {{ $percent }}%
                                </span>
                            </div>
                        </td>
                ...
                <div class="p-4 md:p-6 hover:bg-gray-50 transition-colors duration-150">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-3 md:mb-4">
                        <div class="mb-2 md:mb-0">
                            <h3 class="text-sm md:text-base font-semibold text-gray-900">{{ $monthLabel }}</h3>
                            <p class="text-xs md:text-sm text-gray-600 mt-0.5">{{ $payments->count() }} payment records</p>
                        </div>
                        <div class="text-left md:text-right">
                            <div class="text-base md:text-lg font-bold text-gray-900">Rs. {{ number_format($monthTotal, 2) }}</div>
