...
<div class="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 md:py-6 lg:py-8">
    ...
    <div class="mb-4 md:mb-6 lg:mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="mt-3 sm:mt-0">
                <a href="{{ route('reception.students.create') }}" 
        ...
        <div class="bg-white rounded-xl shadow-md p-3 md:p-4 lg:p-6 border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-3 lg:ml-4">
                    ...
                    <p class="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{{ $students->total() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-3 md:p-4 lg:p-6 border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-3 lg:ml-4">
                    ...
                    <p class="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{{ $totalActiveStudents }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-3 md:p-4 lg:p-6 border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-3 lg:ml-4">
                    ...
                    <p class="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{{ $totalInactiveStudents }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-md p-3 md:p-4 lg:p-6 border border-gray-100">
            <div class="flex items-center">
                ...
                <div class="ml-2 md:ml-3 lg:ml-4">
                    ...
                    <p class="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{{ $totalThisMonthStudents }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-4 md:mb-6 p-3 md:p-4 lg:p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 md:gap-4">
            <!-- Search Box -->
            <form action="{{ route('reception.students.index') }}" method="GET" class="flex-grow lg:max-w-md">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-3">
                ...
                <div class="flex items-center gap-1.5 md:gap-2 bg-gray-100 p-1 rounded-lg">
                    <a href="{{ route('reception.students.index', array_merge(request()->except('status'), ['search' => request('search'), 'rows' => request('rows', 10)])) }}" 
                       class="px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-medium rounded-md transition-all duration-200 {{ !request('status') ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                        All
                    </a>
                    <a href="{{ route('reception.students.index', array_merge(request()->all(), ['status' => 'active'])) }}" 
                       class="px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-medium rounded-md transition-all duration-200 {{ request('status') == 'active' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                        Active
                    </a>
                    <a href="{{ route('reception.students.index', array_merge(request()->all(), ['status' => 'inactive'])) }}" 
                       class="px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-medium rounded-md transition-all duration-200 {{ request('status') == 'inactive' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">
                        Inactive
                    </a>
                </div>
                
                <!-- Rows Per Page -->
                <form action="{{ route('reception.students.index') }}" method="GET" class="relative">
                    <input type="hidden" name="status" value="{{ request('status') }}">
                    ...
                    <select name="rows" onchange="this.form.submit()" class="appearance-none bg-white border border-gray-300 rounded-lg py-1.5 md:py-2 pl-2 md:pl-3 pr-7 md:pr-8 text-xs md:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200">
                        <option value="10" {{ request('rows', 10) == 10 ? 'selected' : '' }}>10 rows</option>
                        <option value="25" {{ request('rows') == 25 ? 'selected' : '' }}>25 rows</option>
                        <option value="50" {{ request('rows') == 50 ? 'selected' : '' }}>50 rows</option>
                    </select>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-2 md:ml-3 lg:ml-4 min-w-0">
                                    ...
                                    <div class="md:hidden mt-1">
                                        <div class="text-xs text-gray-700 truncate">{{ $student->email }}</div>
                                        <div class="text-xs text-gray-500 truncate">{{ $student->student_phone }}</div>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-3 md:px-4 lg:px-6 py-3 md:py-4 hidden md:table-cell">
                            <div class="text-xs md:text-sm text-gray-900 truncate">{{ $student->email }}</div>
                        </td>
                        
                        <!-- Phone Numbers -->
                        <td class="px-3 md:px-4 lg:px-6 py-3 md:py-4 whitespace-nowrap hidden lg:table-cell">
                            <div class="text-xs md:text-sm text-gray-900 truncate">{{ $student->student_phone }}</div>
                            <div class="text-xs text-gray-500 truncate">{{ $student->parent_phone ?? 'No Parent Phone' }}</div>
                        </td>
                        
                        <!-- Status -->
                        <td class="px-3 md:px-4 lg:px-6 py-3 md:py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2 py-0.5 md:px-3 md:py-1 rounded-full text-xs font-medium {{ $student->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <td class="px-3 md:px-4 lg:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm font-medium">
                            <div class="flex items-center space-x-1 md:space-x-2">
                                ...
                                <form action="{{ route('reception.students.destroy', $student) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
