    ...
    <div class="mb-4 md:mb-6 lg:mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="mb-3 sm:mb-0">
                ...
                <p class="mt-1 text-xs md:text-sm text-gray-600">Complete information for {{ $student->first_name }} {{ $student->last_name }}</p>
            </div>
            <div class="mt-3 sm:mt-0 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 md:space-x-3">
                ...
                <a href="{{ route('reception.students.edit', $student) }}" 
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 lg:gap-6">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div class="min-w-0">
                        <h2 class="text-base md:text-lg lg:text-2xl font-bold text-gray-900 truncate">{{ $student->first_name }} {{ $student->last_name }}</h2>
                        <div class="flex flex-wrap items-center mt-1 md:mt-2 gap-1 md:gap-2 lg:gap-4">
                            <span class="inline-flex items-center px-1.5 py-0.5 md:px-2 md:py-1 lg:px-3 lg:py-1 rounded-full text-xs font-medium {{ $student->status == 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                @if($student->status == 'active')
                                <svg class="w-3 h-3 md:w-4 md:h-4 mr-0.5 md:mr-1.5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                </svg>
                                <span class="hidden sm:inline">{{ ucfirst($student->status) }}</span>
                                <span class="sm:hidden">{{ substr(ucfirst($student->status), 0, 1) }}</span>
                                @else
                                <svg class="w-3 h-3 md:w-4 md:h-4 mr-0.5 md:mr-1.5" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <span class="text-xs text-gray-600 hidden sm:inline">ID: {{ $student->id }}</span>
                            <span class="text-xs text-gray-600 truncate">{{ $student->registration_number }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 md:mt-0 grid grid-cols-3 gap-2 md:gap-3 lg:gap-4">
                        <div class="text-center">
                            ...
                            <div class="text-xs md:text-sm font-medium text-gray-900 truncate">{{ optional($student->course)->name ?? 'N/A' }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-xs text-gray-500 mb-0.5 md:mb-1">Center</div>
                            <div class="text-xs md:text-sm font-medium text-gray-900 truncate">{{ optional($student->center)->name ?? 'N/A' }}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-xs text-gray-500 mb-0.5 md:mb-1">Joined</div>
                            <div class="text-xs md:text-sm font-medium text-gray-900">{{ $student->created_at->format('M Y') }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6 mb-3 md:mb-4 lg:mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 lg:gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6 truncate">{{ $student->email }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6">{{ $student->student_phone }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6">{{ $student->parent_phone ?? 'Not provided' }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6">
                            {{ $student->date_of_birth ? \Carbon\Carbon::parse($student->date_of_birth)->format('M d, Y') : 'Not specified' }}
                            @if($student->date_of_birth)
                            <span class="text-gray-500 text-xs">({{ \Carbon\Carbon::parse($student->date_of_birth)->age }} years)</span>
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <p class="text-xs md:text-sm text-gray-900 pl-4 md:pl-6">{{ $student->address ?? 'Not provided' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 lg:gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6 truncate">{{ optional($student->course)->name ?? 'Not assigned' }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6 truncate">{{ optional($student->center)->name ?? 'Not assigned' }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-medium text-gray-900 pl-4 md:pl-6">{{ $student->created_at->format('M d, Y') }}</p>
                        <p class="text-xs text-gray-500 pl-4 md:pl-6">{{ $student->created_at->diffForHumans() }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs md:text-sm font-mono font-medium text-gray-900 bg-gray-50 px-2 py-1 md:px-3 md:py-1.5 rounded-lg pl-4 md:pl-6 truncate">{{ $student->registration_number }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6 mb-3 md:mb-4 lg:mb-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 md:mb-4">
                    ...
                    <div class="flex space-x-1 md:space-x-2">
                        <a href="{{ asset($student->qr_code_path) }}" download 
                ...
                <div class="text-center">
                    <div class="bg-gradient-to-br from-gray-50 to-white border border-gray-100 rounded-lg md:rounded-xl p-3 md:p-4 lg:p-6 mb-3 md:mb-4">
                        <img src="{{ asset($student->qr_code_path) }}" alt="QR Code" class="w-32 h-32 md:w-40 md:h-40 lg:w-48 lg:h-48 mx-auto">
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-lg p-2 md:p-3 lg:p-4">
                        ...
                        <p class="text-xs md:text-sm font-mono text-gray-900 font-medium break-all bg-white px-2 py-1 md:px-3 md:py-2 rounded border border-gray-200">
                            {{ $student->qr_value ?? $student->registration_number }}
                        </p>
                    </div>
                </div>
                @else
                <div class="text-center py-4 md:py-6 lg:py-8">
                    ...
                    <form action="{{ route('reception.qrcodes.assign') }}" method="POST" class="inline-block">
                        @csrf
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-3 md:p-4 lg:p-6">
                ...
                <div class="space-y-2 md:space-y-3">
                    <a href="{{ route('reception.payments.index') . '?student=' . $student->id }}" 
                    ...
                    <form action="{{ route('reception.students.destroy', $student) }}" method="POST" class="block">
                        @csrf
                        @method('DELETE')
                        <button type="submit" 
...
function printQR() {
    ...
    const courseName = "{{ optional($student->course)->name ?? 'No Course' }}";
    const qrCodePath = "{{ asset($student->qr_code_path) }}";
    const qrValue = "{{ $student->qr_value ?? $student->registration_number }}";
