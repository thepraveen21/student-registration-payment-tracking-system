@extends('layouts.reception')

@section('header', 'Schedule')
@section('title', 'Reception Schedule')

@section('content')
<div class="space-y-6">
    <div class="bg-white shadow-sm rounded-lg p-6">
        <h1 class="text-2xl font-bold text-gray-800">Schedule</h1>
        <p class="text-sm text-gray-600">All scheduled events and pending tasks.</p>
    </div>

    <div class="bg-white shadow-sm rounded-lg p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Events</h3>
        <div class="space-y-3">
            @forelse($schedules as $schedule)
                <div class="flex items-center justify-between p-3 rounded-lg 
                    {{ (strtolower($schedule->status ?? $schedule['status']) == 'upcoming') ? 'bg-blue-50' : ((strtolower($schedule->status ?? $schedule['status']) == 'scheduled') ? 'bg-green-50' : 'bg-gray-50') }}">
                    <div class="flex items-center space-x-3">
                        <div class="w-2 h-8 rounded-full 
                            {{ (strtolower($schedule->status ?? $schedule['status']) == 'upcoming') ? 'bg-blue-500' : ((strtolower($schedule->status ?? $schedule['status']) == 'scheduled') ? 'bg-green-500' : 'bg-gray-400') }}"></div>
                        <div>
                            <p class="text-sm font-medium text-gray-800">{{ $schedule->title ?? $schedule['title'] }}</p>
                            <p class="text-xs text-gray-600">
                                {{ ($schedule->time ?? $schedule['time']) }}
                                - {{ $schedule->location ?? $schedule['location'] }}
                                ({{ \Illuminate\Support\Str::limit($schedule->date ?? ($schedule['date'] ?? ''), 10) }})
                            </p>
                        </div>
                    </div>
                    <span class="px-2 py-1 text-xs rounded-full 
                        {{ (strtolower($schedule->status ?? $schedule['status']) == 'upcoming') ? 'bg-blue-100 text-blue-800' : ((strtolower($schedule->status ?? $schedule['status']) == 'scheduled') ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800') }}">
                        {{ $schedule->status ?? $schedule['status'] }}
                    </span>
                </div>
            @empty
                <p class="text-center text-gray-500">No schedules found.</p>
            @endforelse
        </div>
    </div>

    <div class="bg-white shadow-sm rounded-lg p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Pending Tasks</h3>
        <div class="space-y-3">
            @forelse($pendingTasks as $task)
                <div class="flex items-center justify-between p-3 border rounded-lg 
                    {{ (strtolower($task->priority ?? $task['priority']) == 'urgent') ? 'border-red-200' : ((strtolower($task->priority ?? $task['priority']) == 'today') ? 'border-yellow-200' : 'border-blue-200') }}">
                    <div class="flex items-center space-x-3">
                        <input type="checkbox" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        <div>
                            <p class="text-sm font-medium text-gray-800">{{ $task->title ?? $task['title'] }}</p>
                            @if(!empty($task->description ?? ($task['description'] ?? null)))
                                <p class="text-xs text-gray-600">{{ $task->description ?? ($task['description'] ?? '') }}</p>
                            @endif
                        </div>
                    </div>
                    <span class="text-sm {{ (strtolower($task->priority ?? $task['priority']) == 'urgent') ? 'text-red-600' : ((strtolower($task->priority ?? $task['priority']) == 'today') ? 'text-yellow-600' : 'text-blue-600') }}">
                        {{ ucfirst($task->priority ?? $task['priority']) }}
                    </span>
                </div>
            @empty
                <p class="text-center text-gray-500">No pending tasks.</p>
            @endforelse
        </div>
    </div>
</div>

@section('scripts')
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const checkboxes = document.querySelectorAll('input[type="checkbox"]');
        checkboxes.forEach(cb => cb.addEventListener('change', function() {
            const item = this.closest('div.flex.items-center.justify-between');
            if (!item) return;
            if (this.checked) {
                item.style.opacity = '0.6';
                item.style.textDecoration = 'line-through';
            } else {
                item.style.opacity = '1';
                item.style.textDecoration = 'none';
            }
        }));
    });
</script>
@endsection

@endsection
