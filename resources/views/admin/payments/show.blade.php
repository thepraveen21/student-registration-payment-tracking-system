        ...
        <div class="px-6 py-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-lg font-semibold text-gray-900">{{ $payment->course->name }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500">Amount</h3>
                    <p class="mt-1 text-lg font-semibold text-gray-900">Rs.{{ number_format($payment->amount, 2) }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500">Payment Date</h3>
                    <p class="mt-1 text-lg font-semibold text-gray-900">{{ $payment->payment_date->format('M d, Y') }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500">Month Number</h3>
                    <p class="mt-1 text-lg font-semibold text-gray-900">Month {{ $payment->month_number }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500">Recorded By</h3>
                    <p class="mt-1 text-lg font-semibold text-gray-900">{{ $payment->recordedBy->name }}</p>
                </div>
                <div class="md:col-span-2">
                    <h3 class="text-sm font-medium text-gray-500">Notes</h3>
                    <p class="mt-1 text-base text-gray-700">{{ $payment->notes ?: 'N/A' }}</p>
                </div>
            </div>
        </div>
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div class="flex justify-end space-x-3">
                <a href="{{ route('admin.payments.edit', $payment->id) }}" class="inline-flex items-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
