                ...
                <div class="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                    ...
                    <div class="relative">
                        <select name="per_page" id="per-page-filter" class="appearance-none bg-white border border-gray-300 rounded-lg py-2.5 px-4 pr-10 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200">
                            <option value="10" {{ request('per_page') == 10 ? 'selected' : '' }}>Rows: 10</option>
                            <option value="25" {{ request('per_page') == 25 ? 'selected' : '' }}>Rows: 25</option>
                            <option value="50" {{ request('per_page') == 50 ? 'selected' : '' }}>Rows: 50</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <!-- Course & Center -->
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900 font-medium">{{ $student->course->name ?? 'N/A' }}</div>
                            <div class="text-sm text-gray-500">{{ $student->center->name ?? 'N/A' }}</div>
                        </td>
                        
                        <!-- Contact -->
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $student->email }}</div>
                            <div class="text-sm text-gray-500">{{ $student->parent_phone ?? 'No Contact' }}</div>
                        </td>
