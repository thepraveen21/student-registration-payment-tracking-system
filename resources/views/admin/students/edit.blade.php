    ...
    <div class="py-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Update student information for {{ $student->first_name }} {{ $student->last_name }}</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <!-- Form -->
            <form action="{{ route('admin.students.update', $student->id) }}" method="POST" class="px-6 py-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" 
                                  id="address" 
                                  rows="3"
                                  class="mt-1 block w-full px-3 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200">{{ $student->address }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="center_id" id="center_id" 
                            ...
                            @foreach($centers as $center)
                                <option value="{{ $center->id }}" {{ $student->center_id == $center->id ? 'selected' : '' }}>{{ $center->name }}</option>
                            @endforeach
                        </select>
                    </div>
