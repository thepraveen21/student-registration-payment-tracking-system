        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
            ...
            <div class="mt-4 sm:mt-0">
                <div class="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
                    ...
                    <a href="{{ route('admin.reports.payment-wise-students', ['export' => 'pdf']) }}" 
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200 p-6 mb-8">
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-6">
                <div class="text-center">
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-1">{{ $students->count() }}</p>
                </div>
                <div class="text-center">
                    <p class="text-sm font-medium text-blue-600">Total Paid</p>
                    <p class="text-3xl font-bold text-gray-900 mt-1">Rs. {{ number_format($students->sum('total_paid'), 2) }}</p>
                </div>
                <div class="text-center">
                    <p class="text-sm font-medium text-blue-600">Total Due</p>
                    <p class="text-3xl font-bold text-gray-900 mt-1">Rs. {{ number_format($students->sum('total_due'), 2) }}</p>
                </div>
                <div class="text-center">
                    <p class="text-sm font-medium text-blue-600">Overall Balance</p>
                    <p class="text-3xl font-bold text-gray-900 mt-1">Rs. {{ number_format($students->sum('balance'), 2) }}</p>
                </div>
            </div>
        </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {{ $data['student']->first_name }} {{ $data['student']->last_name }} ({{ $data['student']->registration_number }})
                            </td>
                            <!-- Course -->
                            ...
                            <!-- Balance -->
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold {{ $data['balance'] > 0 ? 'text-red-600' : 'text-green-600' }}">
                                Rs. {{ number_format($data['balance'], 2) }}
                            </td>
                            <!-- Status -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium 
                                    {{ $data['payment_status'] == 'paid' ? 'bg-green-100 text-green-800' : ($data['payment_status'] == 'overdue' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                        Total students with payment data: <span class="font-semibold">{{ $students->count() }}</span>
                    </div>
                    <div class="text-sm text-gray-600">
